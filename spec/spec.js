...
describe("all sources of gibs layers must exist and have yesterday images", () => {
    Object.keys(GIBS_LAYERS).forEach(key => {
        it(key + " should work", (done) => {
            console.log('trying to fetch ' + GIBS_LAYERS[key].template.replace('{Time}', '2018-11-03').replace('{TileMatrixSet}', 'GoogleMapsCompatible_Level' + GIBS_LAYERS[key].zoom).replace('{TileMatrix}', '6').replace('{TileRow}', '18').replace('{TileCol}', '34'));
            // date is not relevant
            axios.get(GIBS_LAYERS[key].template.replace('{Time}', '2018-11-03').replace('{TileMatrixSet}', 'GoogleMapsCompatible_Level' + GIBS_LAYERS[key].zoom).replace('{TileMatrix}', '6').replace('{TileRow}', '18').replace('{TileCol}', '34')).then(result => {
                console.log(' success :)');
                expect(result.status).toBe(200); done();
            })
                .catch(error => {
